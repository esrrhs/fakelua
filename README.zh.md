...
- **字符串算法**：`string.trim`/`split`/`starts_with`/`ends_with`/`contains` 底层使用 Boost.Algorithm。
...
FakeLua 在 `src/native/` 下提供 28 个独立 C++ 原生模块，覆盖数学、字符串、表、IO、网络、定时器、事件、随机数、容器、压缩、加密、序列化、数据库、Protobuf、配置解析、日志等领域。
...
| 网络 | `net`（TCP 服务端/客户端）、`http`（Beast HTTP/1.1）、`url`、`timer`、`event` |
...
| 加解密 | `compress`（LZ4/zlib/gzip/Zstd）、`crypto`（MD5/SHA/AES/RC4/Blowfish/DES、UUID、CRC-32） |
