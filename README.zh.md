...
FakeLua 提供完整的核心标准库（`math`、`table`、`string`、`os`、`utf8`、`io`、`net`、`event`、`compress`、`crypto`、`csv`、`json`、`mysql`、`sqlite`、`serialize`、`protobuf`），完全按照独立 C++ 模块解耦设计（`native_math` / `native_table` / `native_string` / `native_os` / `native_utf8` / `native_io` / `native_net` / `native_event` / `native_compress` / `native_crypto` / `native_csv` / `native_json` / `native_mysql` / `native_sqlite` / `native_serialize` / `native_protobuf`），既支持在 Lua 脚本中直接使用，也支持由 CGen 编译器生成的 C 代码进行 Fast-path 直连调用：
...
- **Event 事件库 (`event.*`)**：
  - **订阅管理**：`event.on(event_name, "Package.callback")`（订阅事件，注册函数名回调）、`event.once(event_name, "Package.callback")`（订阅一次，触发后自动移除）、`event.off(event_name, "Package.callback")`（取消订阅）
  - **事件触发**：`event.emit(event_name, arg1, arg2, arg3, arg4)`（触发事件，按订阅顺序依次调用所有处理器，支持最多 4 个可变参数透传）
  - **清理操作**：`event.clear(event_name)`（移除指定事件的所有处理器）、`event.clear_all()`（移除所有事件处理器）
  - **重入安全**：`emit` 内部对处理器列表做快照后迭代，处理器内部可以安全地调用 `on`/`off`/`emit`
  - **典型用途**：游戏逻辑层解耦（单位死亡事件、建筑建造完成事件、回合切换事件等）
- **Compress 压缩库 (`compress.*`)**：
  - **LZ4 压缩**：`compress.lz4_compress(data)`（LZ4 frame 压缩，输出内嵌原始大小）、`compress.lz4_decompress(data)`（解压）
  - **zlib 压缩**：`compress.zlib_compress(data, level?)`（zlib deflate，压缩级别 1-9，默认 6）、`compress.zlib_decompress(data)`（解压）
  - **gzip 压缩**：`compress.gzip_compress(data, level?)`（gzip 格式，压缩级别 1-9，默认 6）、`compress.gzip_decompress(data)`（解压）
  - **Zstd 压缩**：`compress.zstd_compress(data, level?)`（Zstandard，压缩级别 1-22，默认 3）、`compress.zstd_decompress(data)`（解压）
- **Crypto 加密库 (`crypto.*`)**：
  - **哈希函数**：`crypto.md5(data)`（返回 hex 编码 128 位摘要）、`crypto.sha1(data)`（返回 hex 编码 160 位摘要）、`crypto.sha256(data)`（返回 hex 编码 256 位摘要）
  - **编码转换**：`crypto.hex_encode(data)`（二进制转 hex 字符串）、`crypto.hex_decode(hex)`（hex 转二进制）、`crypto.base64_encode(data)`（二进制转 base64，RFC 4648）、`crypto.base64_decode(data)`（base64 转二进制）
  - **AES 对称加密**：`crypto.aes_encrypt_ecb(data, key)` / `crypto.aes_decrypt_ecb`（ECB 模式，data 需 16 字节对齐）、`crypto.aes_encrypt_cbc(data, key, iv)` / `crypto.aes_decrypt_cbc`（CBC 模式，PKCS#7 填充）、`crypto.aes_encrypt_ctr(data, key, iv)` / `crypto.aes_decrypt_ctr`（CTR 模式，流式加密，无需填充）
  - **流/块密码**：`crypto.rc4(key, data)`（RC4 流密码，加密解密同一函数）、`crypto.blowfish_encrypt(key, data)` / `crypto.blowfish_decrypt`（Blowfish ECB）、`crypto.des_encrypt(key, data)` / `crypto.des_decrypt`（DES ECB）、`crypto.triple_des_encrypt(key, data)` / `crypto.triple_des_decrypt`（3DES ECB）
- **CSV 处理库 (`csv.*`)**：
  - **解析**：`csv.decode(str, sep?)`（将 CSV 字符串解析为行的表，每行为字段的 1-indexed 表；字段自动尝试转数字；默认分隔符 `,`；支持引号字段、转义引号 `""`、BOM 跳过、`\r\n` 换行）
  - **生成**：`csv.encode(rows, sep?)`（将行表编码为 CSV 字符串；含引号/分隔符/换行的字段自动加引号；默认分隔符 `,`）
- **JSON 处理库 (`json.*`)**：
  - **编码**：`json.encode(value)`（将 Lua 值转为 JSON 字符串；连续整数 key 1..N 的表转为 JSON 数组，其余转 JSON 对象；浮点数使用 `%.17g` 精度）
  - **解码**：`json.decode(str)`（将 JSON 字符串解析为 Lua 值；`null` → `nil`，整数/浮点数/数组/对象正确转换）
- **MySQL 数据库库 (`mysql.*`)**：
  - **直连模式**：`mysql.connect(config, "Package.on_connect")`（配置 `{host, port, user, password, db}`，异步连接）、`conn:query(sql, "Package.on_result")`（异步查询，结果为行的表）、`conn:stmt_prepare/sql/stmt_execute/stmt_close`（预编译语句）、`conn:tick()`（驱动网络事件）、`conn:close()`
  - **连接池模式**：`mysql_pool.create(config)`（配置额外支持 `pool_size`, `timeout_ms`, `heartbeat_ms`, `max_retries`）、`pool:acquire()/release()/tick()/close()/stats()`
  - 所有操作均为异步回调模式，回调签名为 `function cb(err, result)`
- **SQLite 数据库库 (`sqlite.*`)**：
  - **打开数据库**：`sqlite.open(filename)`（打开或创建 SQLite 数据库，返回 db 对象）
  - **执行 SQL**：`db:exec(sql)`（执行 SQL；SELECT 返回行的表，非 SELECT 返回 nil）
  - **预编译语句**：`db:prepare(sql)`（返回 stmt 对象）、`stmt:bind(...)`（绑定参数，支持 nil/int/float/bool/string）、`stmt:step()`（执行并返回下一行，SELECT 用完返回 nil）、`stmt:reset()`（重置语句保留绑定）、`stmt:columns()`（返回列名表）、`stmt:close()`
  - **辅助方法**：`db:last_insert_rowid()`（最后插入行 rowid）、`db:changes()`（最后影响行数）、`db:close()`
  - 所有操作同步执行，基于 SQLite3  amalgamation 源码
- **Object 对象系统（Lua 侧 API）**：
  - **组管理**：`new_native_group()`（创建组，返回 `group_id`）、`del_native_group(group_id)`（批量销毁组内所有对象，返回销毁数量）
  - **对象创建与查找**：`new_native_obj(group_id, type, id)`（在指定组创建对象）、`get_native_obj(type, id)`（按类型+id 查找）、`new_global_obj(key, type)`（创建全局对象）、`get_global_obj(key)`（查找全局对象）、`del_global_obj(key)`（销毁全局对象）
