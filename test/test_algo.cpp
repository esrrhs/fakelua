...
// For-loop with math param used only as loop bound or step; loop body does NOT use the
// loop variable in any arithmetic expression.  The specialization discovery must detect
// the ForLoop node's EvalType change (T_DYNAMIC → T_INT) as an improvement signal even
...
TEST(algo, count_loop) {
    AlgoRunHelper([](State *s, JITType type, bool debug_mode) {
        ...
        // count_from: n is the for-loop start bound (step = -1); body does c = c + 1.
        ...
        // count_step: n is the for-loop step (bounds fixed 0..100); body does c = c + 1.
        // i = 0, n, 2n, ..., 100 → floor(100/n) + 1 iterations.
        Call(s, type, "test_count_step", r, 5);
        ASSERT_EQ(r, 21);   // 0,5,10,...,100 = 21 steps
        Call(s, type, "test_count_step", r, 10);
        ASSERT_EQ(r, 11);   // 0,10,20,...,100 = 11 steps
    ...
    LuaAlgoRunHelper("./algo/count_loop.lua", [](lua_State *L) {
        ...
        ASSERT_EQ(LuaCall<int>(L, "test_count_from", 5), 5);
        ASSERT_EQ(LuaCall<int>(L, "test_count_from", 1), 1);
        ASSERT_EQ(LuaCall<int>(L, "test_count_from", 10), 10);
        ASSERT_EQ(LuaCall<int>(L, "test_count_step", 5), 21);
        ASSERT_EQ(LuaCall<int>(L, "test_count_step", 10), 11);
        ASSERT_EQ(LuaCall<int>(L, "test_count_step", 1), 101);
    });
}
