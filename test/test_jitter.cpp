...
TEST(jitter, test_infer_for_step_int) {
    JitterRunHelper([](State *s, JITType type, bool debug_mode) {
...
TEST(jitter, test_infer_for_step_dynamic) {
    JitterRunHelper([](State *s, JITType type, bool debug_mode) {
...
// float literal, which hits the different-non-unknown-types branch (line 57)
// and degrades x to T_DYNAMIC.  The post-pass updates the declaration.
TEST(jitter, test_infer_reassign_int_to_float) {
    JitterRunHelper([](State *s, JITType type, bool debug_mode) {
        CompileFile(s, "./jit/test_infer_reassign_int_to_float.lua", {.debug_mode = debug_mode});
        double ret = 0;
        Call(s, type, "test", ret);
        ASSERT_NEAR(ret, 1.5, 0.001);
    });
...
TEST(jitter, test_infer_if_scope_degrade) {
    JitterRunHelper([](State *s, JITType type, bool debug_mode) {
        CompileFile(s, "./jit/test_infer_if_scope_degrade.lua", {.debug_mode = debug_mode});
        int ret = 0;
        Call(s, type, "test", ret, 2);
        ASSERT_EQ(ret, 2);
...
TEST(jitter, test_infer_while_scope_degrade) {
    JitterRunHelper([](State *s, JITType type, bool debug_mode) {
...
TEST(jitter, test_infer_paren_exp) {
    JitterRunHelper([](State *s, JITType type, bool debug_mode) {
        CompileFile(s, "./jit/test_infer_paren_exp.lua", {.debug_mode = debug_mode});
        int ret = 0;
        Call(s, type, "test", ret);
        ASSERT_EQ(ret, 5);
    });
