function test_utf8_offset()
    ...
    -- 空串：offset("", 1) 应为 nil（没有字符可定位）
    local o10 = utf8.offset("", 1)
    if o10 ~= nil then return 0 end

    -- 空串：offset("", 0) 应为 1（Lua 5.4：n=0 从当前位置找字符起始，默认 i=1，返回 1）
    local o11 = utf8.offset("", 0, 1)
    if o11 ~= 1 then return 0 end

    -- 超出末尾：offset("a", 2) 应为 nil（只有 1 个字符）
    local o12 = utf8.offset("a", 2)
