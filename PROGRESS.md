...
| 8 | 常量传播（动态 key 优化） | ✅ | 1 | 327 |

**测试**: 287 个非 pipeline 测试全部通过 + 42 个 pipeline 测试（分组运行全部通过）
...
| 规范内容 | 决策 | 理由 |
|---------|------|------|
| §5.2.2 数组部分 | ❌ 不做 | 表只需支持 hash + 特化 struct |
| §8.1 upvalue/closure 逃逸 | ❌ 不做 | 不支持闭包 |
| §9 偏移分配（手动计算） | ❌ 跳过 | 生成 C 代码，直接用 `a->field` |
| §12.4 元方法 | ❌ 不做 | 不支持元表 |
| §12.5 全局变量 | ❌ 不做 | 全程序常量，读取安全 |

## 已知限制

1. **测试隔离**: pipeline 测试需分组运行（bison parser 跨测试状态泄漏，为测试基础设施问题，不影响编译器功能）

## 结论

**编译器核心功能已完整实现**——按规范 §1-§10 构建了 SSA/CFG/Shape 管线，针对"生成 C 代码 + hash/struct 表 + 无闭包 + 无元表"的目标做了适配。剩余未做的部分均为明确的设计决策（不支持数组、闭包、元表）。

唯一待修复的是测试基础设施的跨测试状态泄漏问题（bison parser 全局状态），不影响编译器功能。
