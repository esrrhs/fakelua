...
private:
    ...
    // MathFuncInfoMap 使用 FunctionSpecInfo 的 {block, params} 子集，
    // 键为函数名，因此无需单独定义 MathFuncInfo 结构体。
    using MathFuncInfoMap = std::unordered_map<std::string, FunctionSpecInfo>;
    ...
    static InferredType RecordType(EvalTypeSnapshot &m, SyntaxTreeInterface *n, InferredType t) {
        m[n] = t;
        return t;
    }

    // 从 LocalVar 节点中解包变量名列表和初始化表达式列表。
    struct LocalVarUnpack {
    ...
    // 数学参数发现的统一类型变化检测器。
    [[nodiscard]] bool CheckArithmeticTypeChanges(const EvalTypeSnapshot &typed_map, const EvalTypeSnapshot &compare_map, const SyntaxTreeInterfacePtr &func_block, bool improvement_mode,
                                                  const std::unordered_map<std::string, std::vector<int>> &math_param_positions) const;

    // 拆分出的细粒度节点检测函数
    [[nodiscard]] bool CheckArithmeticNodeChange(const SyntaxTreeInterfacePtr &node, const EvalTypeSnapshot &typed_map, const EvalTypeSnapshot &compare_map, bool improvement_mode) const;
    [[nodiscard]] bool CheckComparisonNodeChange(const SyntaxTreeInterfacePtr &node, const EvalTypeSnapshot &typed_map, const EvalTypeSnapshot &compare_map, bool improvement_mode) const;
    [[nodiscard]] bool CheckForLoopNodeChange(const SyntaxTreeInterfacePtr &node, const EvalTypeSnapshot &typed_map, const EvalTypeSnapshot &compare_map, bool improvement_mode) const;
    [[nodiscard]] bool CheckCallNodeChange(const SyntaxTreeInterfacePtr &node, const EvalTypeSnapshot &typed_map, const EvalTypeSnapshot &compare_map,
                                           const std::unordered_map<std::string, std::vector<int>> &math_param_positions) const;
    ...
    // 将推断结果转储到临时文件，供调试使用。
    void DumpTypesToTmpFile(const ParseResult &pr, const InferResult &ir) const;
