        ...
        if (stmt->Type() == SyntaxTreeType::LocalVar) {
            ...
            for (size_t i = 0; i < names.size(); ++i) {
                ...
                if (global_type == T_INT) {
                    Out() << "static const int64_t " << name << " = "
                                 << ToInteger(exp_ptr->ExpValue()) << ";\n";
                } else if (global_type == T_FLOAT) {
                    Out() << "static const double " << name << " = "
...
void CGen::CompileTypedIntForLoop(const std::shared_ptr<SyntaxTreeForLoop> &for_stmt) {
    ...
    Out() << GenTab() << "for (; (" << step_var << " > 0) ? (" << ctrl_var << " <= " << end_var << ") : (" << ctrl_var
...
std::string CGen::CompileNativeArithBinop(const SyntaxTreeInterfacePtr &left,
                                                      const SyntaxTreeInterfacePtr &right,
                                                      BinOpKind op_kind,
                                                      InferredType lt,
                                                      InferredType rt) {
...
std::string CGen::CompileNativeCmpBinop(const SyntaxTreeInterfacePtr &left,
                                                    const SyntaxTreeInterfacePtr &right,
                                                    BinOpKind op_kind,
                                                    InferredType lt,
                                                    InferredType rt) {
...
std::string CGen::CompileNativeUnop(const SyntaxTreeInterfacePtr &right,
                                                UnOpKind op_kind,
                                                InferredType rt) {
