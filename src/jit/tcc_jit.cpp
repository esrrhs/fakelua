...
void TccJitter::Compile(const ParseResult &pr, const GenResult &gr, const CompileConfig &cfg) {
    LOG_INFO(s_, "engine", "TCC JIT compile start: {}, {} functions", pr.file_name, gr.function_names.size());
    const auto handle = std::make_shared<TCCHandle>(s_, cfg);
    ::TCCState *s = handle->GetTCCState();

    LOG_DEBUG(s_, "engine", "TCC: compiling {} bytes of C code", gr.c_code.size());
    ...
    LOG_DEBUG(s_, "engine", "TCC: relocating");
    if (tcc_relocate(s) == -1) {
        ThrowFakeluaException(std::format("TCC compile failed, tcc_relocate failed for {}", pr.file_name));
    }

    LOG_DEBUG(s_, "engine", "TCC: registering {} functions", gr.function_names.size());
    ...
    LOG_INFO(s_, "engine", "TCC JIT compilation finished for {}", pr.file_name);
}
