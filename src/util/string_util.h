...
inline const auto g_number_regex = std::regex("^[+-]?[0-9]+(\\.[0-9]+)?([eE][+-]?[0-9]+)?$|^[+-]?0[xX][0-9a-fA-F]+(\\.[0-9a-fA-F]+)?([pP][+-]?[0-9]+)?$");
...
inline std::string JoinString(const std::vector<std::string> &strs, const std::string &sep) {
    ...
    return std::accumulate(std::next(strs.begin()), strs.end(), strs[0], [&](std::string acc, const std::string &item) { return std::move(acc) + sep + item; });
}
