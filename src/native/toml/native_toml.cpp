...
// ::toml::node → CVar
static CVar NodeToLua(State *s, const ::toml::node &node) {
    if (node.is_table()) {
        CVar tbl = table::TableHelper::CreateTable(s);
        auto *t = node.as_table();
        if (t) {
            for (auto it = t->begin(); it != t->end(); ++it) {
                table::TableHelper::SetTableStrId(s, tbl, std::string(it->first.str()).c_str(), NodeToLua(s, it->second));
            }
        }
        return tbl;
    }
...
static ::toml::value<double> MakeTomlFloat(double f) {
...
static bool DetectAndSortArray(std::vector<table::TableKV> &kvs) {
...
static std::unique_ptr<::toml::node> LuaToTomlNode(CVar val, int depth, std::unordered_set<VarTable *> &visited) {
    ...
    switch (val.type_) {
        ...
        case static_cast<int>(VarType::Table): {
            ...
            if (DetectAndSortArray(kvs)) {
                auto arr = std::make_unique<::toml::array>();
                for (auto &kv: kvs) {
                    auto node = LuaToTomlNode(kv.val, depth + 1, visited);
                ...
                for (auto &kv: kvs) {
                    if (kv.key.type_ != static_cast<int>(VarType::Int) && kv.key.type_ != static_cast<int>(VarType::String) && kv.key.type_ != static_cast<int>(VarType::StringId)) {
                        continue;
                    }
...
// Lua Bindings
static CVar TomlDecode(State *s, CVar *args, int n) {
    ...
    try {
        ...
        return NodeToLua(s, result);
    } catch (const ::toml::parse_error &e) {
        ThrowFakeluaException(std::format("TOML parse error: {}", e.description()));
    }
}

static CVar TomlEncode(State *s, CVar *args, int n) {
        ...
        if (a0.type_ == static_cast<int>(VarType::Table)) {
            ...
            if (t) {
                ...
                for (auto &kv: kvs) {
                    ...
                    std::string k = inter::FakeluaToNativeString(nullptr, kv.key);
                    auto node = LuaToTomlNode(kv.val, 0, visited);
            ...
            auto node = LuaToTomlNode(a0, 0, visited);
...
void RegisterTomlLibraryApi(State *s) {
    if (!s) return;
    RegisterNativeFunction(s, "toml.decode", 1, false, TomlDecode);
    RegisterNativeFunction(s, "toml.encode", 1, false, TomlEncode);
}
