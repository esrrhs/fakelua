...
public:
    HttpClientOp(State *state, NativeObject *nat) : io_(state->GetIoContext()), stream_(io_.Get()), resolver_(io_.Get()), lua_state_(state), native_obj_(nat) {
    }

    void Start(std::string method, std::string url, http::fields headers, std::string body, int timeout_ms, unsigned version) {
        ...
        if (!u.has_scheme() || u.scheme_id() != urls::scheme::http) {
            Finish("only http:// URLs are supported");
            return;
        }
        ...
        std::string port = u.has_port() ? std::string(u.port()) : "80";
...
private:
    void OnResolve(beast::error_code ec, tcp::resolver::results_type results) {
        ...
        auto self = shared_from_this();
        auto watch = life_.GetWatch();
        stream_.async_connect(results, [self, watch](beast::error_code conn_ec, tcp::resolver::results_type::endpoint_type) {
            if (!watch.Alive()) return;
    ...
    void OnConnect(beast::error_code ec) {
        ...
        auto self = shared_from_this();
        auto watch = life_.GetWatch();
        http::async_write(stream_, req_, [self, watch](beast::error_code write_ec, std::size_t) {
            if (!watch.Alive()) return;
    ...
    void OnWrite(beast::error_code ec) {
        ...
        auto self = shared_from_this();
        auto watch = life_.GetWatch();
        http::async_read(stream_, buffer_, res_, [self, watch](beast::error_code read_ec, std::size_t) {
            if (!watch.Alive()) return;
            self->OnRead(read_ec);
        });
    }
...
public:
    HttpServerConn(asio::io_context &ioc, tcp::socket sock, int conn_id, int timeout_ms) : stream_(std::move(sock)), conn_id_(conn_id), timeout_ms_(timeout_ms) {
        (void) ioc;
    }
    ...
    void Reply(int status, std::string reason, http::fields headers, std::string body) {
        ...
        auto self = shared_from_this();
        auto watch = life_.GetWatch();
        http::async_write(stream_, res_, [self, watch](beast::error_code ec, std::size_t) {
            if (!watch.Alive()) return;
            self->OnWrite(ec);
        });
    }
...
private:
    void DoRead() {
        ...
        auto self = shared_from_this();
        auto watch = life_.GetWatch();
        http::async_read(stream_, buffer_, req_, [self, watch](beast::error_code ec, std::size_t) {
            if (!watch.Alive()) return;
            self->OnRead(ec);
        });
    }
...
public:
    ...
    void Listen(const std::string &ip, uint16_t port, int backlog, int timeout_ms) {
        timeout_ms_ = timeout_ms;
...
private:
    ...
    void OnAccept(beast::error_code ec, tcp::socket sock) {
        ...
        if (!ec) {
            int id = ++next_id_;
            auto conn = std::make_shared<HttpServerConn>(io_.Get(), std::move(sock), id, timeout_ms_);
            conns_[id] = conn;
            conn->Start(
...
static void ParseRequestOpts(State *s, CVar opts, std::string &method, std::string &url, http::fields &headers, std::string &body, int &timeout_ms, unsigned &version) {
    method = "GET";
    timeout_ms = 10000;
    version = 11;
...
static CVar StartClient(State *s, std::string method, std::string url, http::fields headers, std::string body, int timeout_ms, unsigned version, std::string cb) {
    ...
    op->Start(std::move(method), std::move(url), std::move(headers), std::move(body), timeout_ms, version);
...
static CVar HttpRequest(State *s, CVar *args, int n) {
    ...
    std::string method, url, body, cb;
    http::fields headers;
    int timeout_ms = 10000;
    unsigned version = 11;
    ParseRequestOpts(s, inter::GetNativeArg(s, args, n, 0), method, url, headers, body, timeout_ms, version);
    cb = CVarToString(inter::GetNativeArg(s, args, n, 1));
    if (cb.empty()) ThrowBadArgument(2, "http.request", "callback function expected");
    return StartClient(s, std::move(method), std::move(url), std::move(headers), std::move(body), timeout_ms, version, std::move(cb));
}
...
static CVar HttpGet(State *s, CVar *args, int n) {
    ...
    return StartClient(s, "GET", std::move(url), {}, {}, 10000, 11, std::move(cb));
}
...
static CVar HttpPost(State *s, CVar *args, int n) {
    ...
    return StartClient(s, "POST", std::move(url), std::move(headers), std::move(body), 10000, 11, std::move(cb));
}
...
static CVar HttpServerFn(State *s, CVar *args, int n) {
    ...
    try {
        srv->Listen(ip, port, backlog, timeout_ms);
