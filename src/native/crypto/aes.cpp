...
static const EVP_CIPHER *select_ecb_cipher(AesKeySize key_size, EVP_CIPHER_CTX *ctx) {
    switch (key_size) {
        case AesKeySize::AES_128: return EVP_aes_128_ecb();
        case AesKeySize::AES_192: return EVP_aes_192_ecb();
        case AesKeySize::AES_256: return EVP_aes_256_ecb();
    }
    EVP_CIPHER_CTX_free(ctx);
    ThrowFakeluaException("aes: unsupported key size");
}

static const EVP_CIPHER *select_cbc_cipher(AesKeySize key_size, EVP_CIPHER_CTX *ctx) {
    switch (key_size) {
...
void aes_encrypt_ecb(const uint8_t in[16], uint8_t out[16],
    ...
    const EVP_CIPHER *cipher = select_ecb_cipher(key_size, ctx);
...
std::vector<uint8_t> aes_encrypt_cbc(const uint8_t *data, size_t len,
    ...
    const EVP_CIPHER *cipher = select_cbc_cipher(key_size, ctx);
