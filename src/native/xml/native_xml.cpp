...
static CVar NodeToLua(State *s, pugi::xml_node node) {
    ...
    for (auto attr = node.first_attribute(); attr; attr = attr.next_attribute()) {
        table::TableHelper::SetTableStrId(s, tbl, std::format("_attr_{}", attr.name()).c_str(), inter::NativeToFakeluaString(s, std::string(attr.value())));
    }
...
static void AddXmlAttribute(pugi::xml_node parent, const std::string &attr_name, CVar val) {
    parent.append_attribute(attr_name.c_str()) = ScalarToText(val).c_str();
}

// Append a child element (or #text) under parent. Recurses for tables.
static void AddXmlChild(pugi::xml_document &doc, pugi::xml_node parent, const std::string &key, CVar val, int depth, std::unordered_set<VarTable *> &visited) {
    if (depth > kMaxXmlDepth) {
        ThrowFakeluaException("XML encode: nesting too deep");
    }

    if (key.size() > 6 && key.rfind("_attr_", 0) == 0) {
        AddXmlAttribute(parent, key.substr(6), val);
        return;
    }

    if (key == "_text") {
        parent.append_child(pugi::node_pcdata).set_value(ScalarToText(val).c_str());
        return;
    }
    ...
    switch (val.type_) {
        ...
        case static_cast<int>(VarType::String):
        case static_cast<int>(VarType::StringId):
            child.append_child(pugi::node_pcdata).set_value(ScalarToText(val).c_str());
            break;
        case static_cast<int>(VarType::Table): {
            ...
            if (is_array) {
                ...
                for (auto &kv: kvs) {
                    AddXmlChild(doc, child, "item", kv.val, depth + 1, visited);
                }
            } else {
                for (auto &kv: kvs) {
                    std::string k = inter::FakeluaToNativeString(nullptr, kv.key);
                    AddXmlChild(doc, child, k, kv.val, depth + 1, visited);
                }
            }
            visited.erase(t);
            break;
...
// Lua Bindings
static CVar XmlDecode(State *s, CVar *args, int n) {
...
static CVar XmlEncode(State *s, CVar *args, int n) {
    ...
    try {
        ...
        if (a0.type_ == static_cast<int>(VarType::Table)) {
                ...
                if (kvs[0].val.type_ == static_cast<int>(VarType::Table)) {
                    ...
                    if (vt) {
                        ...
                        for (auto &vkv: vkvs) {
                            ...
                            AddXmlChild(doc, root, vk, vkv.val, 1, visited);
                        }
                        visited.erase(vt);
                    }
                } else {
                    root.append_child(pugi::node_pcdata).set_value(ScalarToText(kvs[0].val).c_str());
                }
            } else {
                auto root = doc.append_child("root");
                for (auto &kv: kvs) {
                    std::string k = inter::FakeluaToNativeString(nullptr, kv.key);
                    AddXmlChild(doc, root, k, kv.val, 1, visited);
                }
            }
            visited.erase(t);
        } else {
            ...
            switch (a0.type_) {
                case static_cast<int>(VarType::Bool):
                    txt = AsVar(a0).GetBool() ? "true" : "false";
                    break;
                case static_cast<int>(VarType::Int):
                    txt = std::to_string(a0.data_.i);
                    break;
                case static_cast<int>(VarType::Float): {
                    char b[64];
                    snprintf(b, sizeof(b), "%.17g", a0.data_.f);
                    txt = b;
                    break;
                }
                case static_cast<int>(VarType::String):
                case static_cast<int>(VarType::StringId):
                    txt = inter::FakeluaToNativeString(nullptr, a0);
                    break;
                default:
                    break;
            }
...
void RegisterXmlLibraryApi(State *s) {
    if (!s) return;
    RegisterNativeFunction(s, "xml.decode", 1, false, XmlDecode);
    RegisterNativeFunction(s, "xml.encode", 1, false, XmlEncode);
}
