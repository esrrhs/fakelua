...
class VarStringHeap {
public:
    // alloc a string, and return the stored VarString.
    VarString *alloc(const std::string_view &str);
...
private:
    ...
    std::unordered_map<std::string_view, VarString *> const_str_map_;
    std::unordered_map<std::string_view, VarString *> tmp_str_map_;
};
