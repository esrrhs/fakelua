...
void interpreter::compile_const_define(const syntax_tree_interface_ptr &chunk) {
    ...
    if (chunk->type() != syntax_tree_type_block) {
    ...
    for (auto &stmt: block->get_stmts()) {
        if (stmt->type() == syntax_tree_type_local_var) {
            auto local_var = std::dynamic_pointer_cast<syntax_tree_local_var>(stmt);
            // TODO
        } else if (stmt->type() == syntax_tree_type_function || stmt->type() == syntax_tree_type_local_function) {
            // skip
        } else {
