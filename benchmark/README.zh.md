...
字节码 VM（`JIT_INTERP`）：8 字节 `Inst`、GNU computed-goto 派发、Lua 5.4 风格整数 `FORPREP`/`FORLOOP` 倒计数、就地写回 `ADD`。生产路径仍是 **JIT_GCC**，不是解释器。

### 运行环境（INTERP 这次）

- 日期：2026-09-16
- 机器：AMD EPYC 7551 32-Core Processor，16 X 2000 MHz CPU s（KVM）
- CPU 缓存：L1d 64 KiB (x16)，L1i 64 KiB (x16)，L2 512 KiB (x16)，L3 65536 KiB (x2)
- 构建模式：**Release**（`-O3 -DNDEBUG`），GCC 16.2.0
- Lua：5.4.7
- FakeLua INTERP：`debug_mode=false`（初始化仍会编 TCC/GCC；计时路径是 `Call(..., JIT_INTERP, ...)`）
- 二进制：`build/bin/bench_mark`

> 绝对耗时强依赖于机器与编译器版本，跨环境不可比。**INTERP vs Lua** = Lua CPU 时间 / INTERP CPU 时间（>1 表示解释器更快）。**INTERP / Lua 耗时** 列是 INTERP / Lua（解释器相对 Lua 慢几倍）。只有本节表内的比值有意义，不要和下面 2026-08-14 的 GCC 表混比。

各场景取最大参数：

### 算法（algo）

| 场景 | 参数 | INTERP vs Lua | INTERP / Lua 耗时 | 备注 |
|------|------|---------------|-------------------|------|
| Fibonacci | n=32 | 0.13x | **7.6x** | 递归 `CALL`；Lua 单次调用便宜得多 |
| GCD | 2147483647/1073741823 | 0.29x | 3.4x | 函数体极小，派发+装箱主导 |
| PowMod | 1234567/7654321/1e9+7 | 0.56x | 1.8x | |
| Sum | n=5M | 0.69x | **1.5x** | 最接近的数值循环（就地 `ADD` + 倒计数 `FORLOOP`） |
| BubbleSort | n=200 | 0.29x | 3.4x | 表下标读写，无 array part |
...
| Vector3 | n=1M | 0.29x | 3.5x | 纯哈希表 vs Lua array part |
| FloatPoly | n=1M | 0.77x | **1.3x** | 紧浮点循环，接近 Sum |

### 字符串（string）

| 场景 | 参数 | INTERP vs Lua | INTERP / Lua 耗时 | 备注 |
|------|------|---------------|-------------------|------|
| StringLen | n=10K | **2.3x** | 0.44x | 工作在 C（`string.len`），解释器更快 |
| StringSub | n=10K | 1.86x | 0.54x | |
| StringRep | n=1000 | 1.08x | 0.93x | 基本持平 |
| StringReverse | n=10K | 1.13x | 0.89x | |
| StringLower | n=10K | **6.2x** | 0.16x | C 原生 `string.lower` |
| StringUpper | n=10K | **6.1x** | 0.16x | 同上 |
| StringByte | n=1000 | 0.77x | 1.3x | |
| StringChar | n=500 | 0.44x | 2.3x | |
| StringFormat | n=500 | 0.78x | 1.3x | |
| StringFind | n=10K | 1.23x | 0.82x | 普通子串 |
| StringGsub | n=1000 | 0.31x | 3.2x | ECMAScript Boost.Regex |
| ToNumber | n=1 | 0.34x | 2.9x | 单次 `Call()` + 装箱 |
| ToString | n=500 | 0.74x | 1.4x | |
| StringFindPattern | n=1000 | 0.25x | 4.0x | ECMAScript 正则 |
| StringGmatch | n=1000 | 0.55x | 1.8x | ECMAScript 正则 |

### 表操作（table）

| 场景 | 参数 | INTERP vs Lua | INTERP / Lua 耗时 | 备注 |
|------|------|---------------|-------------------|------|
| TableInsert | n=5K | 0.29x | 3.5x | 无 array part |
| TableRemove | n=5K | 0.41x | 2.4x | |
| TableConcat | n=1000 | 1.05x | 0.95x | 持平（C 拼接） |
...
| NestedTable | n=10K | 0.25x | 4.0x | 反复 `GETTABLE` |

### 函数调用（function）

| 场景 | 参数 | INTERP vs Lua | INTERP / Lua 耗时 | 备注 |
|------|------|---------------|-------------------|------|
| EmptyCall | n=100K | 0.13x | **7.8x** | 每次 `CallByName` / 进解释器 |
| Recursion | n=25 | 0.17x | **6.0x** | 同 Fibonacci |
| Variadic | n=1 | 0.19x | 5.2x | `select` / 变参 |
| MultiReturn | n=10K | 0.18x | 5.6x | |
| Closure | n=1000 | 0.36x | 2.7x | |
| TailRecursion | n=5K | 0.03x | **34x** | 无 TCO，真 C 递归（需 `ulimit -s unlimited`） |

### GC 与内存压力（gc）

| 场景 | 参数 | INTERP vs Lua | INTERP / Lua 耗时 | 备注 |
|------|------|---------------|-------------------|------|
| TableChurn | n=1000 | **2.2x** | 0.45x | arena，无 GC |
...
| 场景 | 参数 | INTERP vs Lua | INTERP / Lua 耗时 | 备注 |
|------|------|---------------|-------------------|------|
| MathTrig (sin+cos) | n=100K | 0.49x | 2.0x | 原生 math + 解释器循环 |
...
1. **紧整数/浮点循环最接近 Lua**：Sum n=5M 是 Lua 的 **1.5 倍时间**，FloatPoly n=1M 是 **1.3 倍**。剩下主要是带标签的 `CVar` 拷贝和每次寄存器读写的 `boxes[]` 检查。

2. **调用密集大约慢 6–8 倍**：Fibonacci / Recursion / EmptyCall。Lua 的调用是一条 VM 指令；FakeLua INTERP 每次都重新进入 `Call()` / `CallByName` 并装箱参数。
...
6. **正则仍然更慢**（Gsub 为 Lua 时间的 3.2 倍，FindPattern 4.0 倍）：走 ECMAScript Boost.Regex，不是 Lua pattern——和 GCC JIT 同一取舍。

---

## JIT GCC vs Lua / C++（2026-08-14）

下面表格来自**另一次**、**另一台机器**的运行（AMD EPYC 7K62，GCC 15.1.0），**不能**和上面的 INTERP 数字混比。**GCC vs Lua** 表示 FakeLua GCC 相对 Lua 5.4 的加速倍数（>1 表示 GCC JIT 更快）；**GCC vs C++** 表示 FakeLua GCC 与手写 C++ 的耗时比值（<1 表示 FakeLua 更快）。
...
> C++ 参照实现在部分场景会被编译器整体折叠（见下文标注），此时 GCC vs C++ 一列无参考价值。
...
## 完整原始输出（Lua vs INTERP，2026-09-16）

过滤：`BM_Lua_|BM_FakeLua_.*_INTERP`。上面 INTERP vs Lua 表用的是 CPU 时间。
