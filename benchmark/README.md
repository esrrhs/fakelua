...
|------|----------|--------|
| C++ | 4.40 ms | 1x |
| Lua | 144.01 ms | **32.7x** 慢 |
| FakeLua TCC | 324.78 ms | **73.8x** 慢 |
| FakeLua GCC | 17.59 ms | **4.0x** 慢 |

> GCC `-O3` 对递归 Fibonacci 提升约 **18.5x**（相比 TCC），且明显快于 Lua。瓶颈仍在于跨边界调用桥接，但 GCC 已大幅压缩函数调用本身的指令开销。
...
| C++ | 61.0 ns | 1x |
| Lua | 357 ns | **5.9x** 慢 |
| FakeLua TCC | 986 ns | **16.2x** 慢 |
| FakeLua GCC | 148 ns | **2.4x** 慢 |

> GCC 相比 TCC 提升约 **6.7x**，已快于 Lua。
...
| C++ | 91.4 ns | 1x |
| Lua | 558 ns | **6.1x** 慢 |
| FakeLua TCC | 2584 ns | **28.3x** 慢 |
| FakeLua GCC | 199 ns | **2.2x** 慢 |

> GCC 相比 TCC 提升约 **13.0x**，快于 Lua，接近 C++ 的 2x 级别。
...
|------|----------|--------|
| C++ | 1.56 ms | 1x |
| Lua | 29.31 ms | **18.8x** 慢 |
| FakeLua TCC | 147.87 ms | **94.8x** 慢 |
| FakeLua GCC | 1.56 ms | **≈1.0x**（与 C++ 持平） |

> Sum 是最震撼的结果：GCC `-O3` 完全内联并向量化循环体，FakeLua GCC 与原生 C++ **完全持平**，相比 TCC 提升约 **95x**。
...
2. **Lua 稳定在 C++ 的 6–33x 慢**：对于简单的迭代算法倍数相对低；递归密集时倍数随调用深度指数上升。
3. **FakeLua（JIT_TCC）慢于 Lua，FakeLua（JIT_GCC `-O3`）多场景与 C++ 持平或快于 Lua**：同一套脚本在 GCC 后端下，算法类 benchmark 相比 TCC 提升约 **6.7x ~ 95x**。典型样本：
   - Fib(32): TCC 324.78ms → GCC 17.59ms（**18.5x**，快于 Lua 144.01ms）
   - GCD(832040,514229): TCC 986ns → GCC 148ns（**6.7x**，快于 Lua 357ns）
   - PowMod(1234567,7654321,1e9+7): TCC 2584ns → GCC 199ns（**13.0x**，快于 Lua 558ns）
   - Sum(5e6): TCC 147.87ms → GCC 1.56ms（**94.8x**，与 C++ 完全持平）
4. **瓶颈分析更新**：TCC（`-O2`）路径的主要成本是 `FakeluaCallByName` + CVar 装箱/拆箱；GCC（`-O3`）后端通过积极内联/循环展开，大幅压缩脚本内算术/循环开销，Sum 场景已达到与 C++ 持平的极限。这说明后端编译器优化级别对 FakeLua 运行时表现影响极大。
