...
- **Timer 定时器库 (`timer.*`)**：
  ...
  - **共享状态**：测试可通过 `new_global_obj(key, type)` / `get_global_obj(key)` 创建全局 NativeObject（无需 group_id，直接通过 string key 索引，类似 `_G.xxx`），配合 `timer.register_obj_methods(obj)` 注册 `get_int`/`set_int` 方法，供回调记录状态、测试在 main 读取验证（fakelua 无可变全局变量，NativeObject 代替 `_G`）
